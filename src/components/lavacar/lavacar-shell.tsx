...
const NAV: NavItem[] = [
  { href: "/lavacar", label: "Início", icon: Home },
  { href: "/lavacar/caixa", label: "Caixa", icon: Wallet },
  { href: "/lavacar/relatorios", label: "Relatórios", icon: BarChart3 },
  { href: "/lavacar/ajustes", label: "Ajustes", icon: Settings },
];

function isActive(pathname: string, href: string) {
  return href === "/lavacar" ? pathname === "/lavacar" : pathname.startsWith(href);
}
...
function DesktopSidebar({ empresa }: { empresa: Empresa }) {
  ...
  return (
    <aside className="glass sticky top-0 hidden h-dvh w-60 shrink-0 flex-col border-r border-border-soft px-3 py-5 md:flex">
      <Link href="/lavacar" className="mb-6 flex items-center gap-2.5 px-2">
        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-brand to-brand-2 text-white">
          <Car className="h-5 w-5" />
        </span>
